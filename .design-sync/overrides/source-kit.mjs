// forked from design-sync lib/source-kit.mjs — exclude dashboard-ui.tsx from the synth entry (duplicate EmptyState export)
...
// ── REPO FORK: the only behavioural change from the bundled source-kit.mjs ──
...
// The synth entry star-exports every src file, and a name exported by two
// star-exported modules is AMBIGUOUS: esbuild drops it from the bundle
// ENTIRELY — not "one wins" — so window.Capo.EmptyState was undefined and
// every card that composed it rendered blank. An explicit re-export in the
// shim does NOT beat this (verified with a minimal esbuild repro); the only
// fix is for just one module in the graph to export the name.
//
// So dashboard-ui.tsx is kept OUT of the synth entry, and
// .design-sync/named-exports.ts re-exports its six components explicitly.
// Nothing is lost: explicit named re-exports are unambiguous by construction.
// Revisit if the legacy EmptyState is ever renamed in the product, at which
// point this fork can be deleted wholesale.
const ENTRY_EXCLUDE = [/\/dashboard-ui\.tsx$/];
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const comps = srcFiles.filter(
      (p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !ENTRY_EXCLUDE.some((rx) => rx.test(slash(p))),
    );
