...
try {
  ...
  // (6) Day questions must go through `agenda` (the dashboard's own view), not
  // through list_tasks plus the model's own date arithmetic.
  const todayReply = await sendTurn(base.companyId, 'O que temos para hoje?');
  ...
  const buyReply = await sendTurn(base.companyId, 'O que é que eu preciso de comprar para amanhã?');
